...
## 2026-07-30 — Offer-in-chat (Cesar: "send contract... track... a contract in the chat like an icon")
The Alibaba TradeManager / Fiverr custom-offer pattern → R4 centerpiece:
- Quotes/offers render INSIDE the buyer-vendor chat as embedded, soft-shadowed CARDS (icon + price + lead time bold + View details), not text — merging the design addendum's "Quote Timeline in chat" with an explicit vendor action: a "Send offer" button in the chat composer that creates/updates a proposal via the EXISTING proposals API and drops the card into the thread.
