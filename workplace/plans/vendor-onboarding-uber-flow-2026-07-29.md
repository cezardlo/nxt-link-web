...
## 2026-07-29 — CESAR'S PORTAL REDESIGN BLUEPRINT (binding for the S2 rebuild; supersedes the single 12-card line)
Verdict on shipped S1: works but "isn't good" — confirmed (content floats left in a void). Keep: plain-language agreement, unlock-checklist idea, warm regional intro copy, all fields.
New shape: after sign-in, vendor sees welcome + FOUR focused workspaces — 1) Storefront (name/logo→tagline→about→contact, one card each) · 2) Capabilities (industries, client sizes, categories, service areas) · 3) Trust & Proof (certs, case studies, photos, videos, awards) · 4) Agreement & Activation (LAST: "You're almost there" summary + terms as a scannable 4-row TABLE [pay/protected/stay-on-platform/accurate] + one checkbox + "Activate My Storefront"; clickwrap record unchanged).
Persistent CALM CHECKLIST sidebar: plain items w/ green checks, active item violet; NO percentage ring, NO lock icons, no "unlocks" text. Completion state per tab (checkmark draw, brief). Category picker → search+chip interface (S3, shared-component care). "Write it for me" → static TEMPLATE link ("[Company] provides [service] for [industry] in [region]…"); "2 quick questions" prefill = RULES-based branching, never AI. One language at a time (toggle switches ALL). Visual: big bold question headline, small light labels, ONE violet element per screen (the CTA), soft violet-tinted shadows, centered max-width content — per buyer-onboarding-design-spec-2026-07-29.md language.
