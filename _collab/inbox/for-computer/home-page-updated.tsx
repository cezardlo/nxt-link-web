...
export default function Home() {
  ...
  const [fallbackSignals, setFallbackSignals] = useState<IntelSignal[] | null>(null);
  ...
  async function loadTop3() {
    ...
    try {
      ...
      if (data.ok && data.decisions?.length > 0) {
        ...
        // Fallback: /api/decide returned ok:false or no decisions (no AI key)
        await loadFallbackSignals();
      }
    } catch {
      // /api/decide failed — fall back to raw intel signals
      await loadFallbackSignals();
    } finally {
      setLoading(false);
    }
  ...
  async function loadFallbackSignals() {
    try {
      const res = await fetch('/api/intel-signals?limit=3&min_importance=70');
      const data: IntelSignalsResponse = await res.json();
      const signals = data.signals ?? data.findings ?? [];
      if (signals.length > 0) {
        setFallbackSignals(signals.slice(0, 3));
      }
    } catch {
      // non-critical — fallback also failed
    }
  }
      ...
      <div className="relative z-10">
        ...
        <section ref={storyRef} className="py-16 sm:py-20 border-b border-white/[0.06]">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            {!loading && !top3 && fallbackSignals && fallbackSignals.length > 0 && (
              <motion.div
                variants={stagger}
                initial="hidden"
                whileInView="show"
                viewport={{ once: true, margin: '-40px' }}
              >
                <div className="text-[10px] tracking-[0.18em] uppercase font-bold text-nxt-accent/60 mb-4">Top Signals Right Now</div>
                <div className="flex flex-col gap-4">
                  {fallbackSignals.map((signal, i) => (
                    <SignalCard
                      key={`fallback-${i}`}
                      title={signal.title}
                      ...
                      el_paso_score={signal.el_paso_score}
                      el_paso_angle={signal.el_paso_angle}
