...
import { PrivateAccessPrompt } from './PrivateAccessPrompt';
import { isPrivatePath } from '@/lib/privateAccess';
...
export function DockNav() {
  ...
  const [pendingPrivatePath, setPendingPrivatePath] = useState<string | null>(null);
  const pendingLabel = NAV_ITEMS.find((item) => item.href === pendingPrivatePath)?.label;

  function requestPrivateAccess(event: MouseEvent<HTMLAnchorElement>, href: string) {
    if (!isPrivatePath(href)) return;
    event.preventDefault();
    setPendingPrivatePath(href);
  }
  ...
  return (
    <>
      ...
      <PrivateAccessPrompt
        open={pendingPrivatePath !== null}
        sectionName={pendingLabel}
        onClose={() => setPendingPrivatePath(null)}
        onUnlock={() => {
          if (pendingPrivatePath) window.location.href = pendingPrivatePath;
        }}
      />
    </>
  );
