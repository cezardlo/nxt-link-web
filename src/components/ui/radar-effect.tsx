...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      <style>{`
        @keyframes radar-spin { from { transform: rotate(20deg); } to { transform: rotate(380deg); } }
      ...
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(255,102,0,${0.15 - idx * 0.015})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-orange-500/30 bg-black shadow-inner">
        {icon}
      </div>
      <div className="hidden rounded-md px-2 py-1 md:block">
        <div
          className="text-center text-xs font-bold text-white/40"
