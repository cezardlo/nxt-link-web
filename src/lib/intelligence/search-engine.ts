...
function taxonomyMatchBoost(
  parsed: SearchEntityParse,
  classification?: OntologyClassification,
): number {
...
function parseEntities(query: string): SearchEntityParse {
  ...
  const source_types = SOURCE_FILTER_KEYWORDS.filter((item) =>
    normalized.includes(item.keyword),
  ).map((item) => item.source_type);

  const companyPatterns = [
    ...Array.from(
      query.matchAll(/\b([A-Z][a-zA-Z0-9]{2,}(?:\s+[A-Z][a-zA-Z0-9]{2,}){0,3})\b/g),
    ).map((item) => item[1]),
    ...query
...
export async function runHybridAdminSearch(input: {
  ...
  const sourceFilter =
    input.source_types && input.source_types.length > 0
      ? input.source_types
      : parsed.source_types;
  const requiresStrictIndustryCoverage = parsed.industries.some(
    (industry) => tokenize(industry).length >= 2,
  );
  const minIndustryCoverage = requiresStrictIndustryCoverage ? 0.75 : 0.5;

  // Challenges, vendors and pilots previously came from Prisma (SQLite).
  ...
  const pilots: Array<{
    ...
    match: { vendor: { name: string; tags: string } };
  }> = [];

  const [scanRuns, feedbackBoosts] = await Promise.all([
    listOpsScanRuns(50),
  ...
  for (const run of scanRuns) {
    ...
    for (const product of runResult.products) {
      ...
      scanItems.push({
        ...
        problems_solved: Array.isArray(product.problems_solved)
          ? product.problems_solved.slice(0, 3)
          : [],
        ...
        confidence: Number(
          Math.min(
            0.99,
            Number(product.confidence || boostedCombined) * 0.55 + boostedCombined * 0.45,
          ).toFixed(2),
        ),
  ...
  if (scanItems.length === 0 && query.length >= 4) {
    try {
      ...
      for (const product of liveScan.products) {
        ...
        const taxonomyBoost = taxonomyMatchBoost(parsed, product.classification);
        const combined = Number(
          (queryFit * (0.55 + cred * 0.45) * (0.62 + industryCoverage * 0.38)).toFixed(3),
        );
        const boostedCombined = Number(clamp(combined + taxonomyBoost, 0, 0.99).toFixed(3));
  ...
  const challengeItems: HybridResultItem[] = challenges.map((challenge) => {
    ...
    const score = Number(
      (lexicalScore(query, text) * 0.65 + semanticScore(parsed, text) * 0.35).toFixed(3),
    );
    return {
      id: challenge.id,
  ...
  const vendorItems: HybridResultItem[] = vendors.map((vendor) => {
    ...
    const score = Number(
      (lexicalScore(query, text) * 0.6 + semanticScore(parsed, text) * 0.4).toFixed(3),
    );
    return {
      id: vendor.id,
      entity_type: 'vendor',
  ...
  const pilotItems: HybridResultItem[] = pilots.map((pilot) => {
    ...
    const score = Number(
      (lexicalScore(query, text) * 0.58 + semanticScore(parsed, text) * 0.42).toFixed(3),
    );
    return {
      id: pilot.id,
      entity_type: 'pilot',
      ...
      problems_solved: pilot.match.vendor.tags
        ? pilot.match.vendor.tags
            .split(',')
            .map((item) => item.trim())
            .slice(0, 3)
        : [],
  ...
  for (const item of all) {
    ...
    if (item.score > current.score) {
      ...
      current.problems_solved = uniqueStrings([
        ...current.problems_solved,
        ...item.problems_solved,
      ]).slice(0, 4);
    }
  ...
  const deduped = Array.from(dedupedMap.values())
    .map((item) => {
      ...
      return {
        ...
        score: boostedScore,
        confidence: Number(
          clamp(item.confidence + Math.max(0, feedbackBoost * 0.8), 0, 0.99).toFixed(2),
        ),
        risk_score: Number(
          clamp(item.risk_score - Math.max(0, feedbackBoost * 0.7), 0.01, 0.99).toFixed(2),
        ),
      };
    })
    .sort((a, b) => b.score - a.score);
  ...
  return {
    ...
    challenges: challenges
      .filter(
        (challenge) =>
          lexicalScore(
            query,
            `${challenge.title} ${challenge.description} ${challenge.city}`,
          ) > 0,
      )
      .slice(0, 8)
    ...
    vendors: vendors
      .filter(
        (vendor) =>
          lexicalScore(query, `${vendor.name} ${vendor.tags} ${vendor.notes || ''}`) > 0,
      )
      .slice(0, 8)
    ...
    pilots: pilots
      .filter(
        (pilot) =>
          lexicalScore(
            query,
            `${pilot.challenge.title} ${pilot.match.vendor.name} ${pilot.weeklyUpdates}`,
          ) > 0,
      )
      .slice(0, 8)
      .map((pilot) => ({
        ...
        vendorName: pilot.match.vendor.name,
      })),
  };
}
