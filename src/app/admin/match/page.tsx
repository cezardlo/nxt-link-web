...
export default function AdminMatchPage() {
  ...
  async function pushToVendors() {
    if (!requestRef) { setNote('Enter a request ref (REQ-…) to push it to matched vendors.'); return; }
    setLoading(true); setNote('Pushing to matched vendors…');
    try {
      const res = await fetch('/api/match', {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'x-access-code': code },
        body: JSON.stringify({ category, location, request_id: requestRef, include_all: includeAll, push: true }),
      });
      if (res.status === 401) { setAuthed(false); return; }
      const data = await res.json();
      setMatches(data.matches || []);
      if (typeof data.dispatched === 'number') setNote(`Pushed to ${data.dispatched} vendor${data.dispatched === 1 ? '' : 's'} — it's now in their leads inbox.`);
      else if (data.degraded) setNote('Database not connected — connect Supabase to push real vendors.');
      else setNote('Nothing pushed. Make sure the request ref exists and has a buyer email.');
    } finally { setLoading(false); }
  }
        ...
        <div className="m-card m-controls">
          ...
          <div className="m-actions">
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="m-btn ghost" onClick={pushToVendors} disabled={loading || !requestRef} title={requestRef ? 'Send this request to the matched vendors’ leads inbox' : 'Enter a request ref first'}>Push to vendors</button>
...
.m-root{--bg:#F8F7FB;--bg2:#FFFFFF;--surf:rgba(20,19,32,.04);--surf2:rgba(20,19,32,.07);--ink:#141320;--ink-2:#3B3A4A;--ink-3:#615F72;--line:rgba(20,19,32,.08);--line-2:rgba(20,19,32,.14);--brand:#6C5CE0;--brand-2:#4A3DB0;--brand-soft:rgba(108,92,224,.12);--brand-line:rgba(108,92,224,.25);--p2:#4A3DB0;--p3:#4A3DB0;--green:#2F9E6A;--green-soft:rgba(47,158,106,.12);--amber:#C68A28;--amber-soft:rgba(198,138,40,.12);--red:#CE4B43;--red-soft:rgba(206,75,67,.12);
...
.m-gate{max-width:380px;margin:12vh auto;background:var(--surf);border:1px solid var(--line);border-radius:20px;padding:32px;text-align:center;backdrop-filter:blur(12px);box-shadow:0 24px 80px rgba(20,19,32,.12);}
...
.m-mk{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,var(--brand),var(--brand-2));display:grid;place-items:center;margin:0 auto;box-shadow:0 8px 24px rgba(108,92,224,.35);}
.m-mk.sm{width:28px;height:28px;border-radius:8px;box-shadow:0 4px 14px rgba(108,92,224,.3);}
.m-nav{display:flex;align-items:center;justify-content:space-between;padding:13px 28px;background:rgba(248,247,251,.7);backdrop-filter:blur(24px);border-bottom:1px solid var(--line);position:sticky;top:0;z-index:5;}
...
.m-card{background:var(--surf);border:1px solid var(--line);border-radius:20px;padding:30px;backdrop-filter:blur(12px);box-shadow:0 24px 80px rgba(20,19,32,.12);}
...
.m-btn{border:none;cursor:pointer;font:600 14px/1 'Outfit';border-radius:12px;padding:11px 18px;background:var(--brand);color:#fff;box-shadow:0 4px 20px rgba(108,92,224,.35);}
...
.m-result{display:flex;align-items:center;gap:16px;background:var(--surf);border:1px solid var(--line);border-radius:18px;padding:18px 20px;backdrop-filter:blur(12px);box-shadow:0 16px 50px rgba(20,19,32,.10);}
.m-result:hover{border-color:var(--brand-line);}
.m-score{flex:none;width:64px;height:64px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#6C5CE0 var(--p),var(--surf2) 0);position:relative;}
...
.m-root button,.m-root a{transition:background .15s ease,border-color .15s ease,color .15s ease;}
.m-root button:focus-visible,.m-root a:focus-visible,.m-root input:focus-visible,.m-root select:focus-visible{outline:2px solid var(--brand);outline-offset:2px;border-radius:6px;}
