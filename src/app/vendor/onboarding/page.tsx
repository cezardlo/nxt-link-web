    ...
    case 'proof':
      return (
        <>
          ...
          <TrustProofEditor t={t} tf={tf} trust={trust} headingId={headingId} />
        </>
      );
...
function TextField({ label, value, onChange, inputMode, placeholder }: {
  label: string;
  value: string;
...
function TrustProofEditor({ t, tf, trust, headingId }: {
  t: (key: string) => string;
  tf: (key: string, values: Record<string, string | number>) => string;
  trust: TrustTools;
  headingId: string;
}) {
  const noProofYet = trust.certifications.length + trust.caseStudies.length + trust.photos.length === 0;
  const [cert, setCert] = useState<CertificationInput>(() => ({ ...EMPTY_CERTIFICATION_INPUT }));
  const [certFile, setCertFile] = useState<File | null>(null);
  const [caseStudy, setCaseStudy] = useState<CaseStudyInput>(() => ({ ...EMPTY_CASE_STUDY_INPUT }));
  const [photoCaption, setPhotoCaption] = useState('');
  const [photoFile, setPhotoFile] = useState<File | null>(null);
  const [localError, setLocalError] = useState('');
  const [openGroups, setOpenGroups] = useState({ certification: noProofYet, caseStudy: false, photo: false });
  ...
  async function submitCertification(e: React.FormEvent) {
    ...
    setLocalError('');
    trust.setProofError('');
    if (!cert.name.trim()) { setLocalError(t('cert_missing')); return; }
    if (await trust.addCertification(cert, certFile)) {
      setCert({ ...EMPTY_CERTIFICATION_INPUT });
      setCertFile(null);
      if (certFileRef.current) certFileRef.current.value = '';
    }
  }

  async function submitCaseStudy(e: React.FormEvent) {
    e.preventDefault();
    setLocalError('');
    trust.setProofError('');
    if (!caseStudy.title.trim()) { setLocalError(t('case_missing')); return; }
    if (await trust.addCaseStudy(caseStudy)) setCaseStudy({ ...EMPTY_CASE_STUDY_INPUT });
  }
  ...
  async function submitPhoto(e: React.FormEvent) {
    ...
    setLocalError('');
    trust.setProofError('');
    if (!photoFile) { setLocalError(t('photo_missing')); return; }
    if (await trust.addPhoto(photoCaption, photoFile)) {
      setPhotoCaption('');
      setPhotoFile(null);
  ...
  return (
    <div className="vo-proofeditor" data-no-swipe aria-labelledby={headingId}>
      <p className="vo-proof-intro">{t('proof_intro')}</p>
      {(localError || trust.proofError) && (
        <p className="vo-prooferror" role="alert">{localError || t('proof_error')}</p>
      )}

      <details
        className="vo-proofgroup"
        open={openGroups.certification}
        onToggle={(e) => setOpenGroups((old) => ({ ...old, certification: e.currentTarget.open }))}
      >
        <summary>
          <span className="vo-proofgroup-icon" aria-hidden="true"><ShieldCheck size={19} strokeWidth={1.75} /></span>
          <span className="vo-proofgroup-copy"><b>{t('h_certifications')}</b><span>{t('cert_desc')}</span></span>
          <span className="vo-proofgroup-count">{trust.certifications.length}/12</span>
          <ChevronDown className="vo-proofgroup-chevron" size={17} strokeWidth={2} aria-hidden="true" />
        </summary>
        <div className="vo-proofgroup-content">
          {trust.certifications.length === 0 ? <p className="vo-summaryempty">{t('certs_empty')}</p> : (
            <ul className="vo-proofrecords">
              {trust.certifications.map((item) => (
                <li key={item.id}>
                  <span className="vo-proofrecord-mark"><Check size={14} strokeWidth={2.25} aria-hidden="true" /></span>
                  <span className="vo-proofrecord-copy">
                    <b>{item.name}</b>
                    <span>
                      {[item.issuer, item.credential].filter(Boolean).join(' · ') || t('proof_saved')}
                      {item.expires_on ? ` · ${tf('cert_expiry_meta', { date: item.expires_on })}` : ''}
                    </span>
                    {item.image_url && <a href={item.image_url} target="_blank" rel="noreferrer">{t('proof_view_document')}</a>}
                  </span>
                  <button type="button" className="vo-remove" onClick={() => trust.removeCertification(item.id)}>{t('proof_remove')}</button>
                </li>
              ))}
            </ul>
          )}
          <form className="vo-proof-form" onSubmit={submitCertification}>
            <div className="vo-proof-formhead"><b>{t('cert_add')}</b><span>{tf('cert_limit', { n: trust.certifications.length })}</span></div>
            <TextField label={`${t('cert_name')} · ${t('proof_required')}`} value={cert.name} placeholder={t('cert_name_ph')} onChange={(value) => setCert((old) => ({ ...old, name: value }))} />
            <TextField label={`${t('cert_issuer')} · ${t('proof_optional')}`} value={cert.issuer} placeholder={t('cert_issuer_ph')} onChange={(value) => setCert((old) => ({ ...old, issuer: value }))} />
            <TextField label={`${t('cert_credential')} · ${t('proof_optional')}`} value={cert.credential} onChange={(value) => setCert((old) => ({ ...old, credential: value }))} />
            <div className="vo-proof-two">
              <label className="vo-field">{t('cert_issued')} · {t('proof_optional')}
                <input type="date" value={cert.issued_on} onChange={(e) => setCert((old) => ({ ...old, issued_on: e.target.value }))} />
              </label>
              <label className="vo-field">{t('cert_expires')} · {t('proof_optional')}
                <input type="date" value={cert.expires_on} onChange={(e) => setCert((old) => ({ ...old, expires_on: e.target.value }))} />
              </label>
            </div>
            <label className="vo-proof-file">
              <span>{t('cert_document')} · {t('proof_optional')}</span>
              <input ref={certFileRef} type="file" accept="image/png,image/jpeg,image/webp,application/pdf" onChange={(e) => setCertFile(e.target.files?.[0] || null)} />
              <small>{certFile?.name || t('cert_file_help')}</small>
            </label>
            <button type="submit" className="vo-textbtn" disabled={trust.proofBusy === 'certification' || trust.certifications.length >= 12}>
              {trust.proofBusy === 'certification' ? t('proof_saving') : t('cert_add')}
            </button>
          </form>
        </div>
      </details>
      ...
      <details
        className="vo-proofgroup"
        open={openGroups.caseStudy}
        onToggle={(e) => setOpenGroups((old) => ({ ...old, caseStudy: e.currentTarget.open }))}
      >
        <summary>
          <span className="vo-proofgroup-icon" aria-hidden="true"><FileText size={19} strokeWidth={1.75} /></span>
          <span className="vo-proofgroup-copy"><b>{t('h_case_studies')}</b><span>{t('cases_desc')}</span></span>
          <span className="vo-proofgroup-count">{trust.caseStudies.length}/3</span>
          <ChevronDown className="vo-proofgroup-chevron" size={17} strokeWidth={2} aria-hidden="true" />
        </summary>
        <div className="vo-proofgroup-content">
          {trust.caseStudies.length === 0 ? <p className="vo-summaryempty">{t('cases_empty')}</p> : (
            <ul className="vo-proofrecords">
              {trust.caseStudies.map((item) => (
                <li key={item.id}>
                  <span className="vo-proofrecord-mark"><Check size={14} strokeWidth={2.25} aria-hidden="true" /></span>
                  <span className="vo-proofrecord-copy"><b>{item.title}</b><span>{item.result || item.solution || t('proof_saved')}</span></span>
                  <button type="button" className="vo-remove" onClick={() => trust.removeCaseStudy(item.id)}>{t('proof_remove')}</button>
                </li>
              ))}
            </ul>
          )}
          <form className="vo-proof-form" onSubmit={submitCaseStudy}>
            <div className="vo-proof-formhead"><b>{t('case_add')}</b><span>{tf('case_limit', { n: trust.caseStudies.length })}</span></div>
            <TextField label={`${t('case_title')} · ${t('proof_required')}`} value={caseStudy.title} placeholder={t('case_title_ph')} onChange={(value) => setCaseStudy((old) => ({ ...old, title: value }))} />
            <ProofTextarea label={`${t('case_challenge')} · ${t('proof_optional')}`} value={caseStudy.challenge} placeholder={t('case_challenge_ph')} onChange={(value) => setCaseStudy((old) => ({ ...old, challenge: value }))} />
            <ProofTextarea label={`${t('case_solution')} · ${t('proof_optional')}`} value={caseStudy.solution} placeholder={t('case_solution_ph')} onChange={(value) => setCaseStudy((old) => ({ ...old, solution: value }))} />
            <ProofTextarea label={`${t('case_result')} · ${t('proof_optional')}`} value={caseStudy.result} placeholder={t('case_result_ph')} onChange={(value) => setCaseStudy((old) => ({ ...old, result: value }))} />
            <button type="submit" className="vo-textbtn" disabled={trust.proofBusy === 'case-study' || trust.caseStudies.length >= 3}>
              {trust.proofBusy === 'case-study' ? t('proof_saving') : t('case_add')}
            </button>
          </form>
        </div>
      </details>
      ...
      <details
        className="vo-proofgroup"
        open={openGroups.photo}
        onToggle={(e) => setOpenGroups((old) => ({ ...old, photo: e.currentTarget.open }))}
      >
        <summary>
          <span className="vo-proofgroup-icon" aria-hidden="true"><Camera size={19} strokeWidth={1.75} /></span>
          <span className="vo-proofgroup-copy"><b>{t('h_photos')}</b><span>{t('photos_desc')}</span></span>
          <span className="vo-proofgroup-count">{trust.photos.length}/12</span>
          <ChevronDown className="vo-proofgroup-chevron" size={17} strokeWidth={2} aria-hidden="true" />
        </summary>
        <div className="vo-proofgroup-content">
          {trust.photos.length === 0 ? <p className="vo-summaryempty">{t('photos_empty')}</p> : (
            <ul className="vo-proofphotos">
              {trust.photos.map((item) => (
                <li key={item.id}>
                  <div className="vo-proofphoto">
                    {item.image_url ? <img src={item.image_url} alt={item.caption || t('h_photos')} /> : <Camera size={20} strokeWidth={1.5} aria-hidden="true" />}
                  </div>
                  <span>{item.caption || t('proof_saved')}</span>
                  <button type="button" className="vo-remove" onClick={() => trust.removePhoto(item.id)}>{t('proof_remove')}</button>
                </li>
              ))}
            </ul>
          )}
          <form className="vo-proof-form" onSubmit={submitPhoto}>
            <div className="vo-proof-formhead"><b>{t('photo_add')}</b><span>{tf('photo_limit', { n: trust.photos.length })}</span></div>
            <TextField label={`${t('photo_caption')} · ${t('proof_optional')}`} value={photoCaption} placeholder={t('photo_caption_ph')} onChange={setPhotoCaption} />
            <label className="vo-proof-file">
              <span>{t('photo_file')} · {t('proof_required')}</span>
              <input ref={photoFileRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => setPhotoFile(e.target.files?.[0] || null)} />
              <small>{photoFile?.name || t('photo_file_help')}</small>
            </label>
            <button type="submit" className="vo-textbtn" disabled={trust.proofBusy === 'photo' || trust.photos.length >= 12}>
              {trust.proofBusy === 'photo' ? t('proof_saving') : t('photo_add')}
            </button>
          </form>
        </div>
      </details>
...
function ProofTextarea({ label, value, placeholder, onChange }: {
  label: string;
  value: string;
  placeholder: string;
