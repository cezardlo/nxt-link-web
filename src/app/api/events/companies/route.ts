// GET    /api/events/companies?q=&segment=&tech_readiness=  — admin: list target companies
// POST   /api/events/companies                               — admin: add a target company
// PATCH  /api/events/companies   { id, ...fields }           — admin: update a target company
// DELETE /api/events/companies?id=<uuid>                     — admin: remove a target company
// Table: target_companies (private admin intelligence — see
...
import { targetCompanyInput, targetCompanyUpdate } from '@/lib/events/validation';

const TABLE = 'target_companies';
...
function rateLimited(req: Request): NextResponse | null {
  ...
  const rl = checkRateLimit({ key: `events:companies:${ip}`, maxRequests: 30, windowMs: 60_000 });
...
export async function GET(req: Request) {
  ...
  const techReadiness = sp.get('tech_readiness')?.trim() || '';

  try {
    const db = getSupabaseClient({ admin: true });
    let query = db.from(TABLE).select('*').order('name', { ascending: true }).limit(1000);
    if (segment) query = query.eq('segment', segment);
    if (techReadiness) query = query.eq('tech_readiness', techReadiness);
    if (q) {
      // Strip characters that would break the PostgREST or() filter syntax.
      const like = `%${q.replace(/[%_,()]/g, ' ').trim()}%`;
      query = query.or(`name.ilike.${like},industry.ilike.${like},location.ilike.${like}`);
    }
...
export async function POST(req: Request) {
  ...
  const parsed = targetCompanyInput.safeParse(body);
  if (!parsed.success) {
...
export async function PATCH(req: Request) {
  ...
  const parsed = targetCompanyUpdate.safeParse(fields);
  if (!parsed.success) {
    return NextResponse.json({ ok: false, message: firstIssueMessage(parsed.error) }, { status: 400 });
  }
