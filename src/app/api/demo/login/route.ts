...
// Guard: this route mints pre-confirmed accounts with a shared password, so it
// must never be live in production unless explicitly opted in. Enabled when
// ALLOW_DEMO_LOGIN==='true' OR when not running in production.
function demoLoginEnabled(): boolean {
  return process.env.ALLOW_DEMO_LOGIN === 'true' || process.env.NODE_ENV !== 'production';
}

export async function POST(req: Request) {
  if (!demoLoginEnabled()) return NextResponse.json({ ok: false, message: 'Not found' }, { status: 404 });
