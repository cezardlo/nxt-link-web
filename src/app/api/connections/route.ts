...
export async function POST(req: Request) {
  try {
    ...
    // Get recent signals across industries
    const { data: signals } = await db
      .from('intel_signals')
      .select('id, title, signal_type, industry, importance')
      .eq('is_noise', false)
      .order('importance', { ascending: false })
      .limit(30);

    // Get vendors for potential matches
    const { data: vendors } = await db
      .from('vendors')
      .select('name, description, sectors, country')
      .eq('status', 'approved')
      .limit(20);

    const signalList = (signals || []).map(s => s.industry + ': ' + s.title).join('; ');
    const vendorList = (vendors || []).map(v => v.name + ' (' + v.country + '): ' + (v.description || '')).join('; ');
