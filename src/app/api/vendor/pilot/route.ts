...
export async function POST(req: Request) {
  ...
  const { data: opp } = await db.from('quote_requests').select('id, email, public_ref').eq('id', qrId).eq('vendor_id', vendor.id).maybeSingle();
  ...
  const { data, error } = await db.from('pilots').insert({
    ...
    location: String(body.location || '').slice(0, 300) || null,
    scope: String(body.scope || '').slice(0, 2000) || null,
    success_criteria: String(body.success_criteria || '').slice(0, 2000) || null,
  }).select(COLS).single();
...
export async function PATCH(req: Request) {
  ...
  if (typeof body.location === 'string') patch.location = body.location.slice(0, 300) || null;
  if (typeof body.scope === 'string') patch.scope = body.scope.slice(0, 2000) || null;
  if (typeof body.success_criteria === 'string') patch.success_criteria = body.success_criteria.slice(0, 2000) || null;
  if (typeof body.results === 'string') patch.results = body.results.slice(0, 2000) || null;
