...
export async function GET() {
  ...
  const [{ data: brochureRows }, { data: videoRows }, { data: caseRows }, { data: logoRow }] = await Promise.all([
    ...
    db.from('vendor_profiles').select('logo_path, achievements, banner_path, tagline, moderation_status, suspended_until, moderation_reason').eq('id', vendor.id).maybeSingle(),
  ]);
  ...
  const moderation_status = await autoReactivateIfExpired(db, vendor.id, {
    moderation_status: (logoRow?.moderation_status as string) || 'active',
    suspended_until: (logoRow?.suspended_until as string) || null,
  });
  ...
  return NextResponse.json({
    ...
    vendor: {
      ...vendor,
      achievements: (logoRow?.achievements as string[]) || [],
      tagline: (logoRow?.tagline as string) || null,
      moderation_status,
      suspended_until: moderation_status === 'suspended' ? (logoRow?.suspended_until as string) || null : null,
      moderation_reason: moderation_status === 'active' ? null : (logoRow?.moderation_reason as string) || null,
    },
