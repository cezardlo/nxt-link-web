...
export async function GET() {
  ...
  const out = rows.map((r) => {
    ...
    const relevant = cats.some((c) => c && hay.includes(c.split(' ')[0])) || inds.some((i) => i && hay.includes(i.split(' ')[0]));
    return {
      ...
      created_at: r.created_at, relevant, responded: respondedRefs.has(r.public_ref as string),
    };
