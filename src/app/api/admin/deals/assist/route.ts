...
import { calculateFee, FIRST_DEAL_CREDIT_FOUNDING } from '@/lib/fees/engine';
...
export async function POST(req: Request) {
  ...
  if (p.intent === 'log_deal' && p.net_amount) {
    ...
    const reply = `Ready to log: ${p.vendor_name || 'vendor'}${p.buyer_company ? ` → ${p.buyer_company}` : ''} at ${money(p.net_amount)}. Commission ${money(fee.fee)} (${(fee.effectiveRate * 100).toFixed(2)}%)${fee.appliedMaximum ? ', $20k cap applied' : ''}. I've filled the form — review and press “Record deal” to confirm. Founding-vendor credit (−${money(FIRST_DEAL_CREDIT_FOUNDING)}) is optional.`;
