...
export default function SectorIndexPage() {
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        // Load signal counts for all sectors in parallel
        const results = await Promise.allSettled(
          SECTORS.map(sector =>
            fetch(`/api/intel/feed?industry=${encodeURIComponent(sector.slug)}&page_size=1`)
              .then(r => r.json())
              .then(d => ({ slug: sector.slug, count: d.filteredCount ?? 0 }))
          )
        );

        const newStats: Record<string, SectorStats> = {};
        results.forEach((result, i) => {
          const slug = SECTORS[i].slug;
          if (result.status === 'fulfilled') {
            const count = result.value.count;
            newStats[slug] = {
              signalCount: count,
              momentum: count >= 50 ? 'ACCELERATING' : count >= 20 ? 'EMERGING' : 'SLOWING',
            };
          } else {
            newStats[slug] = { signalCount: 0, momentum: 'SLOWING' };
          }
        });
                ...
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {loading && (
                        <div className="mt-1 h-4 w-24 rounded bg-white/[0.04] animate-pulse" />
                      )}
                    </div>
                  </div>
